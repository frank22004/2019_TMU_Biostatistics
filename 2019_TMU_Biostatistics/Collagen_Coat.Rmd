...
4. Table on the Data
```{r}
#allStatus <- paste(df_data$Status.y,df_data$Ingredient)
#df_data <- cbind(df_data,allStatus)

#df_data <- df_data[df_data$TypeI_collagen_μl != 50,]
df_data <- df_data[df_data$Ingredient == "CollagenInStretch_flat",]
df_data$Status.y <- factor(df_data$Status.y, levels = c("UncompressedWithCell","CompressedWithCell"))
#df_data <- df_data[df_data$Ingredient == "polyacryalmide"|df_data$Ingredient == "ScrappingCoating"&df_data$Status.x != "SEM",]
#df_data <- df_data[df_data$識別碼.y=="317"|df_data$識別碼.y=="318",]
...
Mid <- subset.data.frame(df_data, `TypeI_collagen_μl` ==6400)
...
```{r}  
#pstat<- aov(`AreaFriction` ~ Status.y+Ingredient+Status.y*Ingredient, data=df_data)
pstat<- aov(`AreaFriction` ~ Status.y, data=df_data)
...
a_s$dollarsign <- ifelse(a_s$Status.y.Ingredient.p.adj <=0.0001,"$$$$",
                         ifelse(a_s$Status.y.Ingredient.p.adj<=0.001,"$$$",
                                ifelse(a_s$Status.y.Ingredient.p.adj<=0.01,"$$",
                                       ifelse(a_s$Status.y.Ingredient.p.adj<=0.05,"$","ns"))))
a_s$andsign <- ifelse(a_s$Status.y.Ingredient.p.adj<=0.0001,"&&&&",
                         ifelse(a_s$Status.y.Ingredient.p.adj<=0.001,"&&&",
                                ifelse(a_s$Status.y.Ingredient.p.adj<=0.01,"&&",
                                       ifelse(a_s$Status.y.Ingredient.p.adj<=0.05,"&","ns"))))
a_s$p.adj.signif <- ifelse(a_s$Status.y.Ingredient.p.adj<=0.0001,"****",
                         ifelse(a_s$Status.y.Ingredient.p.adj<=0.001,"***",
                                ifelse(a_s$Status.y.Ingredient.p.adj<=0.01,"**",
                                       ifelse(a_s$Status.y.Ingredient.p.adj<=0.05,"*","ns"))))
names(a_s)[names(a_s) == "Status.y.Ingredient.p.adj"] <-  "p.adj"
a_s$`.y.` <- rep("AreaFriction")
...
a_s <- a_s%>%
  add_x_position(dodge = 0.8,group ="Ingredient",x = "Status.y")
a_s
```
...
#dun <- df_data %>%
#  group_by(Ingredient) %>%
#  t_test(`AreaFriction` ~ Status.y, alternative = "greater")%>%
#  tukey_hsd(`AreaFriction` ~ Status.y)%>%
#  add_xy_position(fun = "mean_se", dodge = 0.8,group ="Ingredient",x = "Status.y")
#dun

dun <- compare_means(AreaFriction ~ Status.y, group.by = "Ingredient", data=df_data, method = "t.test")
#dun <- bind_rows(dun,compare_means(AreaFriction ~ Ingredient, group.by = "Status.y", data=df_data, method = "t.test"))
#dun_y <- compare_means(AreaFriction ~ Status.y, data=df_data, method = "t.test")

dun$p.signif <- ifelse(dun$p <=0.0001,"****",
                         ifelse(dun$p<=0.001,"***",
                                ifelse(dun$p<=0.01,"**",
                                       ifelse(dun$p<=0.05,"*","ns"))))
...
#my_xlab <- c("Compressed","Uncompressed")
...
#tiff("fiberdensity.tiff", units = "in", width = 4.5,height = 2.5 , res = 1200)
#tiff("fiberdensity.tiff", units = "in", width = 5,height = 3 , res = 1200)

fiberdensity <- ggboxplot(data = df_data,x="Status.y",y="AreaFriction",fill = "Status.x", title = "Density",size = 0.6)+
...
#                   ,guide = guide_axis(n.dodge = 2)
  ...
  stat_pvalue_manual(dun,hide.ns = TRUE, size = 10, tip.length = 0, bracket.size = 1, step.increase = 0.01, family = "A",label = "{p.signif}",
                     y.position = 0.69
                     )+
#without pA gel
#    stat_pvalue_manual(a_s[1,],hide.ns = TRUE, size = 5, tip.length = 0, bracket.size = 1, step.increase = 0.01,  step.group.by = "Ingredient",y.position =  c(0.82),label = "{dollarsign}",family = "A")+
#  stat_pvalue_manual(a_s[6,],hide.ns = TRUE, size = 5, tip.length = 0, bracket.size = 1, step.increase = 0.01,  y.position = 0.9, step.group.by = "Ingredient",label = "{dollarsign}",family = "A")+
#  stat_pvalue_manual(dun[15,],hide.ns = TRUE, size = 8, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE,y.position = 0.7,step.group.by = "Ingredient")+
#  stat_pvalue_manual(a_s[2:5,],hide.ns = TRUE, size = 4, tip.length = 0, bracket.size = 1, step.increase = 0.05, remove.bracket = TRUE,label = "{dollarsign}",y.position = c(0.75,0.35,0.76))+
#    stat_pvalue_manual(dun[2,],hide.ns = TRUE, size = 12, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE, y.position = 0.68)+
#  stat_pvalue_manual(dun[7,],hide.ns = TRUE, size = 12, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE, y.position = 0.64)+
#    stat_pvalue_manual(dun[15,],hide.ns = TRUE, size = 12, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE, y.position = 0.64)+
#  stat_pvalue_manual(dun[2:3,],hide.ns = TRUE, size = 6, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE,label = "{dollarsign}", y.position = c(0.2,0.7))+
#  stat_pvalue_manual(dun[4,],hide.ns = TRUE, size = 6, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE,label = "{andsign}",y.position = 0.28)+
#  scale_fill_discrete(name = "Process", label=c("Collagen Construct","Collagen Film"))+
#  scale_fill_manual(    values = c("white","gray24"),
#    name = "Collagen Construct", 
#    label=c("Collagen Construct","Collagen Film"))+
...
#    )+
#  scale_fill_discrete(name = "Process", label=c("Collagen Construct","Collagen Film","Polyacrylamide"))+
  labs(subtitle = get_test_label(t_test(`AreaFriction` ~ `Status.y`,data = df_data), detailed = T))+
  theme_bw()+
  border(size = 2)+
  easy_center_title()+  
...
```{r}
fiberdensity
#ggsave("fiberdensity.tiff",device = "tiff", dpi = 700)
```
...
#### Svae the plot
```{r}
ggsave("fiberdensity.png")
```

### 3.1 Boxplot with grouped color for median line
...
p <-ggplot(data = df_data,aes(x=`Status.y`,y=AreaFriction))+
         geom_boxplot(aes(group =interaction(Ingredient,`Status.y`),fill = Ingredient), size=0.8)+
...
p + geom_segment(data=dat, aes(x=xmin, xend=xmax, 
  ...
  stat_summary(aes(group =interaction(Ingredient,`Status.y`)),geom = "errorbar", width = 0.65,fun.min = mean, fun = mean, fun.max = mean,position = position_dodge(0.75), linetype = "dashed", color=c("white","white","black","white"))+
  scale_color_manual(values = c("white","black"))+
  scale_x_discrete(labels=my_xlab)+
  labs(y="Fiber Density (%)", x = "collagen concentration")+
  scale_y_continuous(expand = c(0,0.005),limits = c(0,1),labels = french_percent)+
  stat_pvalue_manual(a_s[1,],hide.ns = TRUE, size = 5, tip.length = 0, bracket.size = 1, step.increase = 0.01,  step.group.by = "Ingredient",y.position =  c(0.82),label = "{dollarsign}",family = "A")+
  stat_pvalue_manual(a_s[6,],hide.ns = TRUE, size = 5, tip.length = 0, bracket.size = 1, step.increase = 0.01,  y.position = 0.9, step.group.by = "Ingredient",label = "{dollarsign}",family = "A")+
  theme_bw()+
  border(size = 2)+
...
ggboxplot(data = df_data, x="Status.y",y="AreaFriction", title = "Density",size = 1, color = "white", fill = "Status.y")+
    scale_x_discrete(labels=my_xlab,guide = guide_axis(n.dodge = 2))+
  labs( y="Fiber Density (%)", x="collagen concentration",expand = c(0,0.005),limits = c(0,0.75)
                     ,labels = percent)+
  stat_pvalue_manual(dun[2:3,],hide.ns = TRUE, size = 5,tip.length = 0, remove.bracket = TRUE,label="{dollarsign}",y.position = c(0.3,0.7), 
    color = "white")+
    stat_pvalue_manual(dun[4:5,],hide.ns = TRUE, size = 5,tip.length = 0, remove.bracket = TRUE,label="{andsign}",color = "white", y.position = 0.25)+
    stat_pvalue_manual(dun[1,],hide.ns = TRUE, size = 10, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE,color = "white")+
  stat_pvalue_manual(dun[6,],hide.ns = TRUE, size = 10, tip.length = 0, bracket.size = 1, step.increase = 0.01, remove.bracket = TRUE, y.position = 0.65788,color = "white")+
  scale_fill_manual(values = c("#00BFC4","#EE6363","green","purple"))+
  theme_bw()+
...
#        prism.ticks.length.y = unit(0.12, "cm"),
        ...
        axis.text.x = element_text(size = 16, color = "white", face = "bold"),
